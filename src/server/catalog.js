/**
 * @file src/server/catalog.js
 * @description Static data: NPC templates, item types, effect descriptors,
 *              and stock encounter-option pools. All data is immutable at
 *              runtime - clone before mutating.
 */

// ─── NPC Templates ───────────────────────────────────────────────────────────
// role: 'aggro' | 'grey' | 'utility'
// stats: server-side authoritative combat/social values
// deathLoot / negLoot: arrays of itemTypeId strings, one is drawn randomly
const NPC_TEMPLATES = {
  // ── AGGRO ──────────────────────────────────────────────────────────────────
  goblin_scout: {
  ...
  skeleton_warrior: {
    ...
    negLoot: [],
    negDifficulty: 99, // cannot be negotiated
  ...
  village_healer: {
    ...
    negDifficulty: 0, // always willing to help
...
// ─── Item Type Registry ───────────────────────────────────────────────────────
// kind: 'consumable' | 'equipment' | 'material' | 'currency' | 'key' | 'token'
// effect: effect descriptor id (see EFFECT_DESCRIPTORS)
// stackable: whether multiple can stack in one slot
// maxStack: maximum per slot
const ITEM_TYPES = {
  // Potions / Consumables
...
// ─── Effect Descriptors ───────────────────────────────────────────────────────
// hook: when the effect fires
//   'on_use' | 'turn_start' | 'turn_end' | 'on_roll' | 'encounter_end'
// scope: how long it lasts
//   'instant' | 'next_roll' | 'next_turn' | 'n_rounds:{n}' | 'encounter' | 'permanent'
// stat: which stat is mutated (null for instant HP effects)
// value: delta applied
...
// Each option: { id, label, tags[], reqRoll, rollStat, difficulty }
// reqRoll: whether this option requires a subsequent roll
// rollStat: which player stat is checked (affects DC calculation)
// difficulty: base DC for resolution (overridden per NPC for negDifficulty)
const OPTION_POOLS = {
  // ── Combat options (used primarily for aggro, available to all roles) ───────
  combat: [
  ...
  // ── Universal fallback options (always available regardless of role) ────────
  universal: [
...
// ─── Encounter Flavor Text Templates ─────────────────────────────────────────
// Used to fill outcome chat messages. {{npc}} and {{player}} are substituted.
...
// ─── Seeded Option Selection ──────────────────────────────────────────────────
// Deterministic pseudo-random from an integer seed using a simple LCG.
 ...
 * Returns 3-5 context-relevant options for a player in an encounter.
 * Options are drawn from pools tagged to npcRole, biased by player stats,
 * always include at least one universal option.
 * @param {string} npcRole - 'aggro' | 'grey' | 'utility'
 * @param {number} seed - integer seed (encounterId hash + playerIndex)
 * @returns {Array} options array, max 5 entries
 */
function getOptionsForEncounter(npcRole, seed) {
  const rng = seededRandom(seed);
  const role = npcRole || 'aggro';

  // Gather all pools relevant to this role
  const allOptions = [
  ...
  // Add use_item option if not already there
...
/**
 * drawLoot
 * Draw 1-2 items from a loot table, returning an array of { id, name } objects.
 * Returns [] if the table is empty or an item id is not in ITEM_TYPES.
 * @param {string[]} lootTable - array of itemTypeId strings
 * @param {number} seed
 * @returns {{ id: string, name: string }[]} drawn items (1-2 entries)
 */
